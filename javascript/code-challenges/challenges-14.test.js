...
const screenForNames = (arr) => {
  ...
  arr.filter(item =>/(Mr|Mrs|Ms|Dr|)\..[A-Za-z]/.test(item));
  return arr
}
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  arr.filter(item =>
    Number(item.mass) > 77).reduce((acc, item) => 
    acc + (acc === '' ? '' : ' - ' ) + item.name,``
    );
    return arr
};
...
/* ------------------------------------------------------------------------------------------------
CHALLENGE 4
Write a function named sortBy that takes in an array of objects, each of which has a particular pro, and sorts those objects by that pro, lowest to highest, returning the same array.
...
const sortBy = (pro, arr) => {
  // Solution code here...
  typeof arr[0][pro] === "string"
//string
    ? arr.sort((a, b) => a[pro].localeCompare(b[pro]))
//nnumber
    : arr.sort((a, b) => a[pro] - b[pro]);
    return arr;
};
