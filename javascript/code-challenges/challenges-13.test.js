...
for (let index = 0; index < arr.length; index++) {
  if(arr[longIndex].length < arr[index].length){
    longIndex = index;
  }
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let newArr= arr.map(ele=>{
    return ele.substring(1,4)+ele.substring(6,9)+ele.substring(10,14);
  });
...
const onlyOddChars = (str) => {
  // Solution code here...
 // return str.filter(ele => str.indexOf(ele) % 2 !== 0)
 let newStr = '';
