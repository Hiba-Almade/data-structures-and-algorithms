...
Write a function named transformToLis that, given an object, returns an array of the key value pairs as html list eles.
...
This function should then raise 2 to the power of the eleing numbers, returning an array of arrays.
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let newArray = input.map(ele => ele.filter(element => element%5===0 ));
  return newArray.forEach(ele => ele.map(element => Math.pow(2, element)));
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter((ele) => ele.gender === 'male' || ele.gender === 'female').map((char) => char.name).join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  let str = data.filter( ele => ele.height).reduce((a, b) => (b.height > a.height ? b : a));
  return str.name;
};
...
describe('Testing challenge 4', () => {
  test('It should return numbers divisible by five, then raise two to the power of the eleing numbers', () => {
